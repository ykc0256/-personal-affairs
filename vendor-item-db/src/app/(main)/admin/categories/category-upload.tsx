...
import { uploadCategoriesExcel, type UploadResult } from "./upload-actions"
import { Button } from "@/components/ui/button"

const INITIAL: UploadResult = { ok: false, message: "", created: 0, updated: 0, errors: [] }

function downloadTemplate() {
  const BOM = "﻿"
  const header = "분류코드,분류명,상위분류코드,정렬순서"
  const samples = [
    "ELEC,전기설비,,1",
    "ELEC_PANEL,배전반,ELEC,1",
    "ELEC_PANEL_LV,저압배전반,ELEC_PANEL,1",
    "MECH,기계설비,,2",
  ]
...
export function CategoryUploadPanel({ onClose }: { onClose: () => void }) {
  const [state, formAction, pending] = useActionState(uploadCategoriesExcel, INITIAL)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-muted-foreground">
            Excel 또는 CSV 파일로 분류 체계를 일괄 등록합니다. 기존 코드가 있으면 업데이트됩니다.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-md border bg-muted/40 px-3 py-2.5 text-xs space-y-1.5">
        ...
        {[
          ["A", "분류코드", "필수. 영대문자·숫자·_ 조합 (예: ELEC_PANEL)"],
          ["B", "분류명", "필수 (예: 배전반)"],
          ["C", "상위분류코드", "선택. 비워두면 최상위 분류로 등록"],
          ["D", "정렬순서", "선택. 비워두면 자동 배정"],
        ].map(([col, name, desc]) => (
      ...
      <form action={formAction} className="space-y-3">
        <div className="flex items-center gap-2">
          <label
            htmlFor="cat-upload-file"
            className="flex flex-1 cursor-pointer items-center gap-2 rounded-md border border-dashed px-3 py-2.5 text-sm text-muted-foreground hover:border-foreground/40 hover:text-foreground transition-colors"
          >
